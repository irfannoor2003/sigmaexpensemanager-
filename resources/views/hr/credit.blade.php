<x-app-layout>
        ...
        <div class="max-w-4xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="order-2 lg:order-1">
                <div class="text-center lg:text-left mb-8">
                    <h1 class="text-3xl font-black dark:text-white">{{__('app.Wallet Funding')}}</span></h1>
                    <p class="text-gray-500 text-sm">{{__('app.Distribute operational credits to managers')}}</p>
                </div>
                ...
                <div class="rounded-[2.5rem] bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 shadow-2xl shadow-pink-500/5 p-8 backdrop-blur-sm">
                    <form action="{{ route('hr.store-credit') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            <label class="block text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2 ml-1">{{__('app.TARGET MANAGER')}}</label>
                            <div class="relative">
    <select name="user_id" required
        ...
        <option value="" disabled selected class="bg-white text-black">{{__('app.Select Manager')}}</option>
    ...
    <div class="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        ...
                        <div>
                            <label class="block text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2 ml-1">{{__('app.CREDIT AMOUNT (PKR)')}}</label>
                            <div class="relative">
                                <span class="absolute left-5 top-1/2 -translate-y-1/2 text-pink-500 font-bold">{{__('app.Rs')}}</span>
                        ...
                        <div>
                            <label class="block text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2 ml-1">{{__('app.ALLOCATION REMARKS')}}</label>
                            <textarea name="remarks" rows="2" placeholder="{{__('app.e.g., Weekly fuel allowance')}}"
            ...
            <div class="order-1 lg:order-2">
                <div class="bg-gray-100/50 dark:bg-white/5 rounded-[2.5rem] p-6 border border-gray-200 dark:border-white/10 h-full">
                    <h3 class="text-sm font-bold dark:text-white mb-6 px-2">{{__('app.Live Balances')}}</h3>
                    <div class="space-y-3">
                        @foreach($users as $user)
                            <div class="flex items-center justify-between p-4 bg-white dark:bg-black/20 rounded-2xl border border-gray-100 dark:border-white/5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-sm font-medium dark:text-gray-300">{{ $user->display_name }}</span>
                                </div>
                    ...
                    <div class="mt-8 p-4 bg-pink-500/5 border border-pink-500/10 rounded-2xl">
                        <p class="text-[10px] text-pink-600 dark:text-pink-400 font-bold uppercase tracking-widest mb-1">{{__('app.SYSTEM NOTE')}}</p>
                        <p class="text-xs text-gray-500 leading-relaxed">{{__('app.Credits are applied instantly')}}</p>
                    </div>
                </div>
            </div>
