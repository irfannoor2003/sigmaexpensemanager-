            ...
            <div class="flex flex-col gap-3">
                <a href="{{ route('manager.dashboard') }}"
                   class="group flex items-center gap-2 text-pink-500 text-[10px] font-black uppercase tracking-[0.2em] hover:gap-3 transition-all duration-300">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="group-hover:-translate-x-1 transition-transform">
                    ...
                    <table class="w-full text-left text-sm border-collapse">
                        <thead>
                            <tr class="text-gray-400 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/[0.02]">
                                <th class="px-8 py-6 font-bold tracking-widest uppercase text-[9px]">Expense Details</th>
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                            ...
                            <tr class="group hover:bg-pink-500/[0.02] transition-all duration-300">
                                <td class="px-8 py-5">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-gray-900 dark:text-white group-hover:text-pink-500 transition-colors">{{ $item->title }}</span>
                                        <span class="text-[10px] text-gray-400 font-mono">Ref: #EXP-{{ str_pad($item->id, 5, '0', STR_PAD_LEFT) }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-5">
                                    <span class="px-3 py-1 rounded-lg bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-400 text-[10px] font-black uppercase tracking-tight">
                                ...
                                <td class="px-8 py-5 font-mono font-black text-pink-600 dark:text-pink-400 text-base">
                                    -Rs. {{ number_format($item->amount) }}
                                </td>
                                <td class="px-8 py-5 text-right">
                                    <span class="inline-flex px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest {{ $item->status === 'approved' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-orange-500/10 text-orange-500' }}">
                                        {{ $item->status }}
                                    </span>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="5" class="px-8 py-20 text-center">
                                    <div class="flex flex-col items-center justify-center opacity-30">
                                        <svg class="w-12 h-12 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                                        <p class="text-xs font-black uppercase tracking-[0.4em]">No Records Found</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
